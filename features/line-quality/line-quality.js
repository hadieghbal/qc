...
// ===================================================================
// 💡 تابع حل مشکل: فعال کردن انتخابگر تاریخ شمسی (jdp) - نسخه اصلاح شده
// این تابع ابتدا تاریخ را تنظیم می‌کند و سپس منتظر کتابخانه برای فعال‌سازی تقویم می‌شود.
// ===================================================================
function setupJalaliDatePicker(dateInput) {
  ...
  // --- مرحله ۱: تنظیم فوری مقدار تاریخ (مستقل از کتابخانه) ---
  ...
  // --- مرحله ۲: تلاش برای فعال‌سازی ویجت تقویم ---
  const maxTries = 40; // چک کردن برای حداکثر 2 ثانیه (هر 50 میلی‌ثانیه)
  let currentTry = 0;

  // استفاده از setInterval تا زمانی که window.jdp آماده شود
  const interval = setInterval(() => {
    if (window.jdp) {
      clearInterval(interval); // متوقف کردن چک کردن

      // اتصال jdp به فیلد ورودی
      window.jdp.render(dateInput, {
        usePersianDigits: true,
        format: "YYYY/MM/DD",
      });
      console.log("Jalali Date Picker widget initialized successfully.");
    } else if (++currentTry >= maxTries) {
      clearInterval(interval); // متوقف کردن بعد از تلاش‌های مجاز
      console.error(
        "Date picker library (jdp) did not load in time after multiple checks. The input will work as a standard text field."
      );
    }
  }, 50);
}
