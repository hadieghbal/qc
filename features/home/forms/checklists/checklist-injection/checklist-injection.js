...
export function init() {
  console.log(
    "Checklist Injection Initialized! (v: High Quality Image Export)"
  );
  ...
  // تابع کمکی برای دانلود فایل (مشابه فرم ضایعات)
  ...
  // تابع کمکی برای اینلاین کردن استایل‌ها (مشابه فرم ضایعات)
  ...
  // ===== تابع جدید اکسپورت عکس (مشابه فرم ضایعات) =====
  async function exportToImage() {
    ...
    if (exportButton) {
      ...
      exportButton.innerHTML = "در حال آماده سازی تصویر...";
    }
    ...
    // اگر کانتینر وجود نداشت یکی می‌سازیم (در فایل HTML شما ممکن است نباشد)
    ...
    // تنظیم استایل‌های ضروری برای مخفی بودن اما قابل رندر بودن
    printContainer.style.position = "absolute";
    printContainer.style.left = "-9999px";
    printContainer.style.visibility = "visible";
    printContainer.style.opacity = "1";
    printContainer.style.background = "#fff"; // زمینه سفید
    ...
    try {
      ...
      const buildFullPageHTML = (itemsHTML) => {
        // هدر اصلی (فرض بر این است که آی‌دی main-header در صفحه اصلی دارید)
      ...
      const createItemCardHTML = (item, index) => {
        ...
        const headerHTML = `
            <div class="summary-card-header" style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:8px; border-bottom:1px solid #eee; padding-bottom:8px;">
                <h2 class="summary-card-title" style="font-size:16px; margin:0; color:#0d6efd;">
                    ${index + 1}. ${item.deviceName}
                    <span style="font-weight:bold; color:#333; font-size:14px; display:block; margin-top:4px;">${partNamesText}</span>
                </h2>
        ...
        // جزئیات (مواد، مستربچ، وزن، سیکل، سریال)
        let detailsHTML = "";
      ...
      const canvas = await html2canvas(printContainer, {
        useCORS: true,
        scale: window.devicePixelRatio || 2, // کیفیت بالا مثل فرم ضایعات
